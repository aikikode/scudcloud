...
class Wrapper(QWebView):
    ...
    def call(self, function, arg=None):
        ...
        if arg is None:
            arg = ''
        return self.page().currentFrame().evaluateJavaScript('ScudCloud.{}({});'.format(function, arg))

    def urlChanged(self, q_url):
        url = q_url.toString()
        # Some integrations/auth will get back to /services with no way to get back to chat
        if Resources.SERVICES_URL_RE.match(url):
            self.systemOpen(url)
            self.load(QUrl('https://{}/messages/general'.format(q_url.host)))
        else:
    ...
    def openChannel(self, menuitem, _):
